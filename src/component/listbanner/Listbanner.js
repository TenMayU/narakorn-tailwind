...
export default function Listbanner(){
  const kalasin = "kalasin"
  const sahatakhan = "sahatakhan"
    return(
        <>
 <h1 className='text-xl  font-mono sm:2xl md:text-3xl lg:text-4xl xl:text-5xl  2xl:text-6xl'>สำรวจสถานที่ท่องเที่ยวเด่น</h1>
        <div id="list-banner" className='  container w-full min-h-[200px] h-fit  flex flex-col mt-2 sm:mt-4 md:mt-8 lg:mt-16 xl:mt-20 2xl:mt-24  rounded-lg overflow-hidden'>
        
         <Link href={`/listlandmark/${kalasin}`} id="imgcontainer" className='w-full h-20 lg:h-32 xl:h-44 2xl:h-56 flex-col items-center  relative flex overflow-hidden  group cursor-pointer '>
         ...
         <Link href={`/listlandmark/${sahatakhan}`} id="imgcontainer" className='w-full h-20 lg:h-32 xl:h-44 2xl:h-56  flex-col items-center  relative flex overflow-hidden  group cursor-pointer '>
